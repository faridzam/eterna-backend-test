...
export function createAppConfiguration(environment: NodeJS.ProcessEnv): AppConfiguration {
  const nodeEnvironment = environment.NODE_ENV ?? 'development';
  const isProduction = nodeEnvironment === 'production';
  const databaseUrl = environment.DATABASE_URL ?? (isProduction ? undefined : 'postgresql://stockflow:stockflow@localhost:5432/stockflow?schema=public');
  ...
  if (corsOrigins.length === 0 || !corsOrigins.every((origin) => URL.canParse(origin))) {
    throw new Error('CORS_ORIGINS must contain one or more valid origins.');
  }

  const sessionTokenPepper = environment.SESSION_TOKEN_PEPPER ?? 'development-only-session-pepper-change-me';
  ...
  if (databaseUrl === undefined || !URL.canParse(databaseUrl) || new URL(databaseUrl).protocol !== 'postgresql:') {
  ...
  return {
    ...
    sessionDurationMs: positiveInteger(environment.SESSION_DURATION_HOURS, 24 * 7, 'SESSION_DURATION_HOURS') * 60 * 60 * 1000,
    sessionTokenPepper,
    taxRateBasisPoints: positiveInteger(environment.TAX_RATE_BASIS_POINTS, 1100, 'TAX_RATE_BASIS_POINTS'),
  };
}
